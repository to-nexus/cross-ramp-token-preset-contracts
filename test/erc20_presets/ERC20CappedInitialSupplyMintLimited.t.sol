...
import {ERC20CappedInitialSupplyMintLimited} from "../../src/erc20/presets/ERC20CappedInitialSupplyMintLimited.sol";
import {Test, console} from "forge-std-1.10.0/src/Test.sol";

contract ERC20CappedInitialSupplyMintLimitedTest is Test {
    ERC20CappedInitialSupplyMintLimited public token;
    ...
    uint256 constant INITIAL_SUPPLY = 1000 * 10 ** 18; // 1K tokens initial supply
    ...
    function test_mint_upto_limit() public {
        ...
        assertEq(token.totalSupply(), INITIAL_SUPPLY + LIMIT);
    }
    ...
    function test_revert_when_mint_exceeds_cap() public {
        ...
        vm.prank(forge1);
        token.mint(user1, LIMIT - INITIAL_SUPPLY);
        for (uint256 i = 1; i < periodsNeeded; i++) {
