...
contract ERC20MintLimited is ERC20PeriodMintLimit {
    constructor(
        ...
        uint256 limit
    ) ERC20Base(owner, forges, name, symbol, decimals) ERC20PeriodMintLimit(duration, offsetSeconds, limit) {}
...
contract ERC20MintLimitedPreset is ERC20BasePreset {
    ...
    function deployCode(bytes memory initialData) external pure override returns (bytes memory) {
        ...
        return
            abi.encodePacked(code(), abi.encode(owner, forges, name, symbol, decimals, duration, offsetSeconds, limit));
    }
}
